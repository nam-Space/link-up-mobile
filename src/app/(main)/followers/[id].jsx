...
const Followers = () => {
    ...
    useEffect(() => {
        setLoadingFollows(Array(followers.length).fill(false));
    ...
    const handleDeleteFollow = async (item, index) => {
        ...
        if (res.success) {
            ...
            //     let newFollowers = prevFollowers.filter(
            //         (fl) => fl.followerId != currentUser?.id
            //     );
            //     return newFollowers;
            // });
            setUser((prevUser) => {
                ...
                newUser.following = [...newUser.following].filter((it) => {
                    return it?.followeeId != item?.followerId;
                });
                return newUser;
            });
        } else {
                ...
                <View style={styles.container}>
                    ...
                    {followers.length > 0 && (
                        <FlatList
                            ...
                            renderItem={({ item, index }) => {
                                const checkUserFollow =
                                    currentUser?.following?.some(
                                        (fl) => fl.followeeId == item.followerId
                                    );
                            ...
                            onEndReached={() => {
                                if (hasMore)
                                    getFollowers(PAGE_SIZE_FOLLOWER, id);
                            }}
